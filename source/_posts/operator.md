...
最近公司晚上直播网课『重学前端』，这里写一下感想。。。

然后最近写需求遇到个问题，当 `count` 为 `null` 或者 `undefined` 的时候，界面展示`...`，而当 `count` 为 0 或大于 0 的数字时，展示 `count`，一个很简单的问题嘛，我的写法是 `count || count === 0`，看起来一切正常，自信的去 review，然后发现似乎可以替换为一个更简洁的写法`count >= 0`，然后自测过程中，发现界面上的`...`不见了，排查了半天 bug，发现似乎是因为 count 为 null，而 null>=0 这段代码导致的。
